...
def _fence_chunk(fence_lines: int, prose_words: int) -> str:
    fence = "```python\n" + ("x = compute_value(1)  # exemplo\n" * fence_lines) + "```"
    prose = " ".join(f"palavra{i}" for i in range(prose_words))
    return f"{prose}\n\n{fence}"


def test_chunk_is_fence_dominated_is_off_by_default():
    from zettel.extractor import chunk_is_fence_dominated

    cfg = _make_config()
    assert cfg.extraction.max_fence_ratio == 1.0
    assert not chunk_is_fence_dominated(cfg, _fence_chunk(60, 5))  # ~97% code


def test_chunk_is_fence_dominated_above_the_ratio():
    from zettel.extractor import chunk_is_fence_dominated

    cfg = _make_config(max_fence_ratio=0.65)
    assert chunk_is_fence_dominated(cfg, _fence_chunk(60, 5))


def test_chunk_without_fence_is_never_gated():
    from zettel.extractor import chunk_is_fence_dominated

    cfg = _make_config(max_fence_ratio=0.01)  # as aggressive as it gets
    assert not chunk_is_fence_dominated(cfg, "prosa comum sem nenhum bloco de codigo")
    assert not chunk_is_fence_dominated(cfg, "")


def test_process_chunk_gated_by_fence_ratio_makes_no_llm_call(tmp_path, monkeypatch):
    """The whole point: the verdict is knowable without paying for it (#154)."""
    import json
    ...
    chunk_row = dict(chunk_row)
    chunk_row["text"] = _fence_chunk(60, 5)

    def exploding_call_llm(*args, **kwargs):
        raise AssertionError("o gate deveria ter evitado a chamada ao LLM")

    monkeypatch.setattr("zettel.extractor.call_llm", exploding_call_llm)
    prompt_parts = load_prompt_parts(cfg.prompts_path / "literature_note.md")
    candidates, output = _process_chunk(cfg, db, None, object(), chunk_row, prompt_parts, "h")

    assert candidates == []
    assert output is None
    persisted = json.loads(db.get_chunk("@Book2024::ch000::abc")["summary_json"])
    assert persisted["chunk_status"] == "rejected"
    assert persisted["rejection_category"] == "fragmented"
    # An operator must be able to tell this from the model's own verdict.
    assert "deterministico" in persisted["rejection_reason"]
    assert db.get_chunk("@Book2024::ch000::abc")["literature_note_path"] is None
    db.close()


def test_process_chunk_below_the_fence_ratio_still_calls_the_llm(tmp_path, monkeypatch):
    import json
    ...
    cfg.extraction.max_fence_ratio = 0.65
    chunk_row = dict(chunk_row)
    chunk_row["text"] = _fence_chunk(2, 300)  # mostly prose
    ...
    def fake_call_llm(llm, user, system=None, **kwargs):
        ...
        return json.dumps(
            {
                "chunk_status": "rejected",
                "rejection_reason": "sem densidade",
                "rejection_category": "narrative",
                "summary": "Resumo qualquer do trecho analisado pelo modelo.",
                "key_concepts": [],
                "candidates": [],
            }
        )
    ...
    monkeypatch.setattr("zettel.extractor.call_llm", fake_call_llm)
    prompt_parts = load_prompt_parts(cfg.prompts_path / "literature_note.md")
    _process_chunk(cfg, db, None, object(), chunk_row, prompt_parts, "h")

    assert called, "chunk majoritariamente em prosa deve seguir para o LLM"
    db.close()
