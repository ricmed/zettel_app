...
def _add_source(database, source_id: str, citekey: str, title: str) -> None:
    database.upsert_source(
        ...
        authors=["Autor"],
        year=2020,
        file_checksum=f"chk-{citekey}",
        origin_path=f"/inbox/{citekey}.pdf",
        origin_type="pdf",
    )
...
def db(tmp_path: Path, cfg: AppConfig):
    ...
    database.upsert_chunk(
        "@Autor2020Livro::ch000::aaaa1111",
        "@Autor2020Livro",
        "@Autor2020Livro::ch000",
        "texto",
        "chk1",
    )
    ...
    specs = [
        (
            ...
            "Dropout treina sub-redes e as media na inferencia",
            "Falha quando a rede ja e pequena",
            5,
            ["regularizacao", "dropout"],
            ["The 5 Whys"],
        ),
        (
            NOTE_B,
            "Batch norm e covariate shift",
            "Batch norm reduz covariate shift interno",
            "",
            4,
            ["normalizacao"],
            [],
        ...
        (
            NOTE_C,
            "Attention sem recorrencia",
            "Attention modela dependencias longas sem recorrencia",
            "",
            3,
            ["attention"],
            [],
    ...
    for note_id, title, thesis, limits, relevance, tags, frameworks in specs:
        ...
        if note_id == NOTE_A:
            meta["decision_rules"] = [
                "Quando a rede for grande, use dropout, porque reduz variancia"
            ]
            meta["anti_patterns"] = [
                "O que evitar: dropout na inferencia - por que falha: escala errada"
            ]
...
def test_cheatsheet_is_honest_when_nothing_was_stated(cfg, db, tmp_path):
    ...
    path.write_text("corpo", encoding="utf-8")
    db.upsert_note(
        note_id="01HDDDDDDDDDDDDDDDDDDDDDDD",
        source_id="@Outro2021",
        path=str(path),
        title="Sem julgamento",
        body=_note_body("Uma tese qualquer"),
...
def test_topic_slice_unions_matching_mocs(cfg, db, tmp_path):
    db.upsert_moc(
        "01HMMMMMMMMMMMMMMMMMMMMMM1",
        topic="Redes Neurais",
        body=f"[[ZTL - {NOTE_A} - a]]",
    )
    db.upsert_moc(
        "01HMMMMMMMMMMMMMMMMMMMMMM2",
        topic="Redes Neurais",
        body=f"[[ZTL - {NOTE_C} - c]]",
    )
    ...
    entries = build_term_map(
        [
            TermSource(
                "n1",
                "notes/a.md",
                thesis="Dropout funciona como ensemble implicito de sub-redes",
            ),
        ]
    )
