...
def score(
    ...
    for stratum in (STRATUM_CONTESTED, STRATUM_STRUCTURAL, STRATUM_ACCEPTED):
        ...
        # Against each item's CURRENT verdict, not the side its stratum was drawn from.
        # On the export key the two coincide; on a key regenerated by a re-run extractor
        # an item drawn from a rejected stratum may now be accepted, and counting "human
        # keeps" there would report a disagreement the extractor no longer makes.
        disagree = sum(
            1
            for lab, item in judged_here
            if (lab.verdict == KEEP) != (item.llm_verdict == "accepted")
        )
...
def render(result: ExtractionScore, report: ReadReport) -> str:
    ...
    for s in result.strata:
        ...
        disagree = round(s.disagreement_rate * s.judged)
        lines.append(
            f"  {s.stratum:<11} pop={s.population:>4} amostra={s.sampled:>3} "
            f"julgados={s.judged:>3} '?'={s.unjudgeable}  humano guarda {s.human_keep}  "
            f"discorda do LLM: {disagree}/{s.judged} = {pct(s.disagreement_rate)}  ({ci})"
        )
