...
async def new_note(
    ...
    try:
        ...
        if from_lit_path and (
            ".." in from_lit_path or from_lit_path.startswith("/") or ":\\" in from_lit_path
        ):
            from_lit_path = ""
...
async def biblio_preview(request: Request):
    ...
    try:
        ...
        if not payload["abnt_reference"]:
            payload["warning"] = "Informe o tipo de documento para montar a referência ABNT."
        return payload
...
async def create_note(request: Request):
    ...
    try:
        ...
        if parsed["mode"] in {"LIT_INDEX", "LIT_GRANULAR"} and not selected["source_id"]:
        ...
        if parsed["mode"] == "ZTL_FROM_LIT":
            ...
            thesis = preflight_from_lit(
                ...
                parsed,
                ref,
                llm_ok=_connect_llm_ok(svc.cfg),
            )
            return post_job(
                request,
                "manual-ztl-from-lit",
        ...
        result = scaffold_manual_note(
            ...
            key_concepts=(parsed.get("lit_concepts") if parsed["mode"] == "LIT_GRANULAR" else None),
            candidates=(
        ...
        return _form_page(
            ...
            status_code=201,
            result=result,
            next_step=_next_step(svc.cfg, db, result),
            selected=selected,
        )
        ...
        granular = ("1" if posted_granular == "1" else "") if note_type == "LIT" else "1"
        return _form_page(
            request,
            db,
            status_code=400,
