...
def init(
    config: Annotated[Optional[str], typer.Option(
        "--config", "-c", help="Caminho para config.yaml",
    )] = None,
    vault: Annotated[Optional[str], typer.Option(
        "--vault", help="Caminho do vault (override)",
    )] = None,
    inbox: Annotated[Optional[str], typer.Option(
        "--inbox", help="Caminho do inbox (override)",
    )] = None,
    reset: Annotated[bool, typer.Option(
        "--reset", help="Apagar e recriar todas as bases de dados",
    )] = False,
):
...
def reindex(
    ...
    collection: Annotated[Optional[str], typer.Option(
        "--collection", help="Reindexar apenas: sources|chunks|permanent_notes|mocs",
    )] = None,
    force: Annotated[bool, typer.Option(
        "--force", help="Resetar a colecao antes de repovoar",
    )] = False,
    yes: YesOption = False,
):
...
def rebuild(
    ...
    what: Annotated[str, typer.Option(
        "--what", help="vault | chroma | all",
    )] = "vault",
    force: Annotated[bool, typer.Option(
        "--force", help="Sobrescrever arquivos existentes (nunca notas manuais)",
    )] = False,
    ...
    if what in ("vault", "all"):
        ...
        metrics_table(
            "Rebuild vault" + (" (dry-run)" if dry_run else ""), vstats,
        )
    ...
    if what in ("chroma", "all"):
        ...
        metrics_table(
            "Rebuild chroma", rstats, key_label="Colecao", value_label="Vetores",
        )
