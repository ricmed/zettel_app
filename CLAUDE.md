...
**Phase 2b (review.py)**: Selective approval. Approved drafts move to `20_Literature/{Citekey}/LIT - AuthorYear - pNNN - topic-NNNN.md`, are embedded into Chroma collection **`literature_notes`** (interpretation only — the source excerpt block is stripped), the index is refreshed with labeled wikilinks, and concepts are deduped then marked `approved` for connect. Rejected drafts are deleted and never indexed as literature notes.
