...
**Phase 1 (harvester.py + paging.py)**: Scans `data/inbox/`, extracts text (Docling for PDF, native for MD), generates citekeys, creates SRC (`SRC - AuthorYear - slug.md`) + literature **index** (`LIT - AuthorYear - slug.md`, no `@`, no `-index` suffix). Resolves content-start paging (HITL, `--content-start-file` / `--content-start-book`, or silent heuristic when non-interactive): PDF **file** page where content begins + **printed** number on that page (book front matter → printed 1; journal article whose PDF starts at file p.1 may be printed p.200 via header/footer or biblio `pages: 200-210`; handouts that start at 1 stay 1=1). `--skip-paging` forces file p.1 = printed p.1. Pages before the start produce no chunks. Native Markdown has **no pages** (locator is `section_path`; stray digits are not treated as pages). Chunks text via structural H3–H6 + LangChain splitter. Infers `page_in_file` from a Docling page-break map in the **same Markdown dialect as the chunks** (`<!-- zettel:page-break -->` in `extracted_text`; per-page Docling export if no breaks; PyMuPDF only as fallback). Regex on chunk text runs only when there is no page map and the source is not Markdown. Then `page_in_book = file - start_file + start_book` (multi-page chunks use the **first** page). Docling `prov.page_no` is the PDF file index, not the printed number. Runs three-layer duplicate detection. Indexes raw chunks into Chroma `chunks` (for dedupe/FTS); does **not** create per-chunk LIT files yet. `zettel set-paging` repairs the printed offset on an existing source without re-calling the LLM (renames granular LIT files when the page token changes). Wrong `page_in_file` on sources harvested before the Docling map needs a re-harvest (or `rechunk` when `extracted_text` already has page-break markers). Opt-in `--dump-chunks` (harvest/rechunk) writes one markdown file per source under `data/cache/chunk-dumps/` (or `--dump-dir`) with the persisted chunk text plus paging/`section_path`/overlap metadata — the same payload `extract` will see. `zettel dump-chunks --source-id @Citekey` reexports already-harvested sources without reprocessing. Opt-in `--dump-extraction` (harvest) writes `sources.extracted_text` (Docling Markdown / native MD, H1–H6 intact; PDF may include `<!-- zettel:page-break -->`) under `data/cache/extraction-dumps/` (or `--dump-extraction-dir`) as soon as the text is persisted; `zettel dump-extraction --source-id @Citekey` reexports without re-extracting.
