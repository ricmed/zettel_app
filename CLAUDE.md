...
- **Absolute relevance floor (`RelevanceFloorConfig` in config.py)**: RRF's fused score is purely *positional* — dense kNN always returns the N closest notes in the corpus regardless of whether any are actually relevant, so a totally off-topic question gets a similarly "confident" score to a genuinely answerable one. `_apply_relevance_floor` in retrieval.py gates on the raw vector similarity instead (`1 - distance/2 >= min_vector_similarity`, default `0.70`, empirically calibrated on this project's corpus/embedding model — retune per corpus). A hit found via BM25 bypasses the floor unconditionally (`bm25_hit_bypasses_floor`, default True) since a real lexical match is evidence a kNN "closest available" hit isn't.
