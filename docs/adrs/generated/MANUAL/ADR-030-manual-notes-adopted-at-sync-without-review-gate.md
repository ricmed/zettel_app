...
For permanent notes, `create_permanent_from_literature` offers two paths from a literature note. Without `--llm` it writes a pre-filled scaffold (thesis, definition, `literature_ref`, `source_ref`, `source_locator`, supporting excerpt) and deliberately writes **no** `concepts` row, so a later `zettel connect` cannot produce a duplicate note for the same idea. With `--llm` it derives a `PermanentNoteCandidate` from the note's own sections, persists it as an `approved` concept, and calls `connector.run_connect(..., origin="manual")` — inheriting hybrid RAG, Prompt 2, ULID minting, typed `note_connections`, `auto-backlinks` maintenance, the deterministic LLM cache and cost accounting. The only change required in the connector was threading an `origin` parameter so manual output stays distinguishable from pipeline output. When that call yields no note, the concept row survives as `approved`, so `zettel connect` retries it later.
