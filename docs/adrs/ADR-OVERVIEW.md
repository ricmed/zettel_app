...
└─ ADR-036 (Topic index: routing, not representation)
...
├─ ADR-032 (cli.py -> cli/ package)
├─ ADR-035 (zettel skill: flat Agent Skill export)
└─ ADR-037 (Pre-flight de custo LLM)

QA-WRITING (3 ADRs)
├─ ADR-028 (LangGraph StateGraph article orchestration)
├─ ADR-029 (article_graph.py → article_graph/ package)
└─ ADR-038 (Evals offline do ask: replay + veredictos)
...
| ADR | Title | Depends On | Feeds Into |
|-----|-------|-----------|------------|
| **ADR-022** | FastAPI+Jinja2 | ADR-008, ADR-023, ADR-026 | ADR-039, ADR-040 |
