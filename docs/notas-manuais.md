...
- **LIT granular manual é adotada por completo** ([ADR-030](adrs/generated/MANUAL/ADR-030-manual-notes-adopted-at-sync-without-review-gate.md)): o sync cria a linha de `chunks` que uma nota escrita à mão nunca teve (mais um capítulo sintético `Manual` por fonte), e a adiciona ao bloco `auto-lit-index` do índice da fonte. A partir daí ela é indistinguível de uma LIT aprovada pelo pipeline e pode virar uma ZTL. **Não** aparece em `ask`/`article`: o `Retriever` pontua notas permanentes, nunca LIT (ADR-036) — e desde a remoção da coleção `literature_notes` não há sequer um vetor de LIT. Notas manuais **não passam pelo portão de aprovação** — entram como `persisted` direto, porque o portão existe para conteúdo gerado por LLM.
