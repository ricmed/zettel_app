...
- **GPU sem `bfloat16` nativo.** O Docling carrega o `CodeFormulaV2` em `bfloat16`. Em GPUs que não suportam esse tipo nativamente (ex.: RTX série 20, compute capability 7.5), o torch o emula gastando *mais* memória que `float32`. Numa RTX 2060 de 6 GB, um PDF de 7 páginas chegou a 8,63 GB de pico e 13 minutos, transbordando para a RAM do sistema, e uma colheita real morreu com corrupção de heap sem traceback. O harvest detecta a GPU e carrega o modelo em `float16` nesse caso: 2,45 GB e 2 minutos, com texto idêntico ao de `float32`. Nenhuma configuração a fazer.
