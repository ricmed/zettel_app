...
- Só alvos que são **notas permanentes** roteiam. Linhas sem `note_id` nunca viram semente — o Retriever pontua ZTL, não LIT.
...
O índice se materializa no `garden`/`sync-manual` (MOC). Um vault já maduro pega tudo de uma vez com `zettel reindex`, que refaz o escopo MOC e limpa leftovers de fonte (como já faz com o FTS5). Decisão e alternativas recusadas: [ADR-036](adrs/generated/RETRIEVAL/ADR-036-topic-index-routing-not-representation.md).
